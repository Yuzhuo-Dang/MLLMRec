...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            item_feat_file_path = os.path.join(dataset_path, config['image_text_feature_file'])
            if os.path.isfile(item_feat_file_path):
                self.item_feat = torch.from_numpy(np.load(item_feat_file_path, allow_pickle=True)).type(
                    torch.FloatTensor).to(self.device)
                
            user_feat_file_path = os.path.join(dataset_path, config['user_preference_feature_file'])
            if os.path.isfile(user_feat_file_path):
                self.user_preference_feat = torch.from_numpy(np.load(user_feat_file_path, allow_pickle=True)).type(torch.FloatTensor).to(self.device)

            assert self.v_feat is not None or self.t_feat is not None or self.item_feat is not None, 'Features all NONE'
